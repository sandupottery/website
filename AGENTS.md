...
- **Each home title line carries a band of the ground colour behind it (`.linea`), and that is a contrast device, not decoration.** The title crosses the photograph, and a photograph can be dark, light or busy exactly there. Gradients and `mix-blend-mode` were both tried and rejected by the client, and both depend on the image; this does not. It needs one block per line (`righe()` in `src/lib/testo.tsx`) — with `<br>` and `box-decoration-break: clone` the bands of consecutive lines overlap and the lower one clips the letters above it, because the titles are set at `line-height: 0.9`.
...
- **Everything painted with `--ground` needs the same `background-color` transition.** `--ground` is rewritten every animation frame while the home scrolls, so a surface without the transition snaps to the new value while `body` is still nine tenths of a second behind it — which is visible as a rectangle of a different colour for the whole length of the change. `body`, `#margine` and `.soglia .titolo .linea` all carry the identical `0.9s cubic-bezier(0.4, 0, 0.2, 1)`; a new surface that reads `--ground` gets it too, or it flickers.
