...
- **Leaving a room by the wordmark suppresses the two shared names, and it has to.** «Torna» lands on the threshold, where the photograph has a title to shrink into; the wordmark lands at the top of the home, where it has nothing. An unpaired `view-transition-name` does not fade out *with* its page — the browser lifts it out and dissolves it **over** the page arriving underneath, which meant the room's full-bleed photograph hanging above the opening statement for most of a second. `Cima` marks the exit (`html[data-uscita="marchio"]`) in a capturing click handler, because the snapshot is taken during that commit; `globals.css` then drops the names with `!important` (React writes them inline, and inline wins otherwise); `Scorrimento` clears the mark on arrival, after the snapshot, or the next «Torna» would lose its morph too. What is left is the whole page dissolving, which is what the wordmark means: from there you do not go back, you start again.
