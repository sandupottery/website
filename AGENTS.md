...
- **The sticky caption needs both an explicit height on `.soglia-a` and a `margin-bottom` on `.testo`, and losing either fails silently.** A sticky element's travel is its containing block's height minus its own margin box. The height on the link is the box the caption stops inside — without it the containing block becomes the whole ribbon and all six captions pile up at the top of the page, which looks like a z-index bug and is not one. The `margin-bottom: calc(var(--alt) * 0.45)` is what makes it let go halfway down its photograph; `45%` would be wrong, because percentage margins resolve against inline size.
